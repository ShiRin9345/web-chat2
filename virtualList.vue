<template>
  ...
  >
    <n-flex
      ...
      justify="center"
      class="box-border absolute-x-center pt-10px"
    >
...
const ESTIMATED_ITEM_HEIGHT =
  props.estimatedItemHeight || DEFAULT_ESTIMATED_HEIGHT; // 每项的预估高度
    ...
    const visibleKeys = new Set(
      props.items
        .slice(
          ...
          visibleRange.value.end + BUFFER_SIZE + 1
        )
...
const totalHeight = computed(() => {
  ...
  // 如果有累积高度缓存，直接使用最后一个值
  if (
    accumulatedHeights.value.length > 0 &&
    accumulatedHeights.value.length === props.items.length
  ) {
  ...
  return props.items.reduce((total, item) => {
    return (
      total +
      (heights.value.get(item.message?.id?.toString()) || ESTIMATED_ITEM_HEIGHT)
    );
  }, 0);
});
...
watch(
  ...
  (newVisibleData) => {
    ...
    const currentVisibleIds = new Set(
      newVisibleData.map((item) => item.message?.id?.toString()).filter(Boolean)
    );
...
const cleanupInvisibleDOMNodes = () => {
  ...
  // 找出不再可见的项目ID
  const invisibleIds = Array.from(previousVisibleIds.value).filter(
...
const getOffsetForIndex = (index: number) => {
  ...
  for (let i = 0; i < index; i++) {
    const itemHeight =
      heights.value.get(props.items[i].message?.id?.toString()) ||
...
const updateVisibleRange = () => {
  ...
  while (total < clientHeight + OVERSCAN_SIZE * 2 && end < props.items.length) {
    const itemHeight =
      heights.value.get(props.items[end].message?.id?.toString()) ||
      ESTIMATED_ITEM_HEIGHT;
    total += itemHeight;
    end++;
  }
  ...
  const topOffset =
    (!props.isLoadingMore && props.isLast) ||
    (props.isLoadingMore && !props.isLast)
      ? 32
      : 0;
  const newOffset =
    getOffsetForIndex(start) +
    (props.isLoadingMore && !props.isLast ? LOADING_OFFSET : 0) +
    topOffset;
  offset = newOffset;
...
const emitVisibleItems = () => {
  const visibleIds = visibleData.value
    .map((item) => item.message?.id?.toString())
